 ...
 *********************************************/
 ...
 subject to {
   ...
   forall (i in 1..m, j in 1..n) sum (k in 1..m, l in 1..n) y[i][j][k][l] == x[i][j];
   
   forall (i in 1..m, j in 1..n, k in 1..m, l in 1..n) y[i][j][k][l] <= x[i][j];
 ...
 main {
   ...
   model.generate();
   //var data = new IloOplDataSource("MinFragmentation.dat");
   ...
   while (Math.abs(currentObj) > eps ){ 
      ...
      var nextSource = new IloOplModelSource("Fragmentation.mod");
